...
def test_branching():
    ...
    assert e.state.context_view._pstr_branch_info() == "IP: 0x40119a <main+0x51 in sym_exec.elf (0x119a)>\tCond: <Bool %s[127:120] == 80>\n\tVars: frozenset({'%s'})\n" % (argv1.args[0], argv1.args[0]), "Branch info not as expected"
    ...
    assert b'PASSWORD' in e.simgr.deadended[1].solver.eval(argv1, cast_to=bytes)
