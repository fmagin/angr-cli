...
def test_max_depth():
    ...
    assert state.context_view._pstr_register("RAX",
                                             state.regs.rax) == 'RAX:\t0x2000 ──> 0x3000 ──> 0x4000 ──> 0x5000 ──> 0x6000 ──> 0x7000 ──> <BV64 0x8000>'
...
def test_loop():
    ...
    assert state.context_view._pstr_register("RAX", state.regs.rax) == "RAX:\t0x1337 ──> 0x4242 ──> 0x1337 ──> 0x4242 ──> 0x1337 ──> 0x4242 ──> <BV64 0x1337>"

def test_cc():
    state = proj.factory.blank_state()
    state.regs.rax = 0x2000
    colored_ast = state.context_view._color_code_ast(state.regs.rax)[0]
    assert colored_ast == '0x2000'
