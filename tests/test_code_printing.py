...
def test_linear_disass_valid_code():
    ...
    nose.tools.assert_true("--> 0x401050:	endbr64	" in state.context_view._pstr_current_codeblock(linear_code=True), "'--> 0x401050:	endbr64	' not in code")

def test_linear_disass_symbolic_code():
    state = proj.factory.blank_state(addr=0x1337)
    nose.tools.assert_equal(state.context_view._pstr_current_codeblock(linear_code=True), "Instructions are symbolic!")

def test_entry_print():
    state = proj.factory.entry_state()
    state.context_view: angrcli.plugins.ContextView.context_view.ContextView
    nose.tools.assert_equal("0x401050:	endbr64\t", state.context_view._pstr_current_codeblock().split("\n")[1], "First code line not as expected")
    nose.tools.assert_equal(state.context_view._pstr_current_codeblock(), state.context_view._pstr_code(), "Code pane must be equal to current codeblock on entry state")
